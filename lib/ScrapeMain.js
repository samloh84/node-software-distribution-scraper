 ...
 * @param options.startingUrls
 * @param options.parseCallback
 * @param options.filterCallback
 * @param options.patterns
 *
 * @returns {Promise<R2|R1>|Promise<R>}
 */
ScrapeMain.scrapeLinks = function scrapeLinks(options) {
 ...
 * @param options
 * @param options.language
 * @param options.shortlistUrlsCallback
 * @param options.downloadOptions
 * @param options.sortingCallback
 * @returns {Promise<R>|Promise<R2|R1>}
 */
ScrapeMain.downloadAndProcessLinks = function downloadAndProcessLinks(options) {
    var language = _.get(options, 'language');
    ...
    return Promise.resolve(shortlistUrlsCallback(urls))
        .then(function (shortlistedUrls) {
            return Promise.map(shortlistedUrls, function (versionUrlAttributes) {
                var version = versionUrlAttributes.version;
                var url = versionUrlAttributes.url;
                var filename = versionUrlAttributes.filename;

                if (_.isNil(filename)) {
                    filename = _path.basename(_.get(_url.parse(url), 'pathname'));
                }
                ...
                return retrieve(url)
                    ...
                    .then(function (props) {
                        ...
                        var sortingResult = sortingCallback(_.merge({}, versionUrlAttributes, props));
                        var outputPath = sortingResult.path;
                        var data = sortingResult.data;
                        _.set(output.urls, outputPath, data);
                    });
            }, {concurrency: 10});
        });
};
...
ScrapeMain.generateDockerfiles = function generateDockerfiles(options) {
...
_.assign(ScrapeMain, _.mapValues(ScrapeMain, Promise.method));
